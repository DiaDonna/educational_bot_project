...
def show_all_history(message: Message) -> None:
    ...
    # если пользователь хочет посмотреть историю поиска за определенный период, то устанавливаем состояние search_from
    #  и отправляем первый календарь для ввода начальной даты поиска
